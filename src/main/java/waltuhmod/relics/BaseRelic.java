...
import static waltuhmod.Waltuhmod.relicPath;
...
public abstract class BaseRelic extends CustomRelic {
    ...
    //To use a basegame relic image, just pass in the imagename used by a basegame relic instead of the POWER_ID.
    //eg. "calendar.png"
     ...
     * @param ID the relic's POWER_ID
     * @return the relic's POWER_ID, to allow use in super constructor invocation
     */
    private static String testStrings(String ID) {
        ...
        if (text == null) {
            throw new RuntimeException("The \"" + ID + "\" relic does not have associated text. Make sure " +
                    "there's no issue with the RelicStrings.json file, and that the POWER_ID in the json file matches the " +
                    "relic's POWER_ID. It should look like \"${modID}:" + GeneralUtils.removePrefix(ID) + "\".");
        }
        return ID;
    }
